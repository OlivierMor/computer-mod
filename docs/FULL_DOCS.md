...
For the best experience we recommend playing with Create, this mod, and the **Aeronautics** addon
together: Aeronautics adds real flying machines, and this mod gives them a brain, which is what makes
autonomous aircraft possible. It is a recommendation, not a requirement. Every other Create addon
works with this mod too, because the Sensor reads addon blocks generically, as the
"Works with other mods" section below explains.

## What you can build

- **Autonomous planes with Aeronautics.** A computer reads the aircraft's instruments, holds altitude
  and heading, follows a route between airport beacons, and lands while you sit back. Airports
  broadcast their coordinates over channels; every plane in the sky can navigate to them.
- **With just Create:** a factory that watches its own stock and switches production lines on and off,
  a self-refilling farm, a train depot that reports what every station is doing, an overstress alarm
  that flashes before your network grinds to a halt.
- An alarm that flashes a lamp when a chest is nearly full or a tank runs low.
- A smart vehicle or contraption you steer with the keyboard, like a plane, train, or rover, with the
  computer enforcing speed limits and safety cut-offs while you drive.
...
from any mod, even one with no documented API. That is why Aeronautics, our recommended companion, and
every other Create addon work with this mod out of the box. See
